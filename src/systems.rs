...
pub fn respawn_pipes(mut pipe_query: Query<(&mut Transform, &TopOrBottom), With<Pipe>>) {
    let mut rng = rand::thread_rng();
    let top_y_options = vec![-150, -50, -250];
    let bottom_y_options = vec![-350, -250, -450];
    for (mut pipe, top_or_bottom) in &mut pipe_query {
        let chosen_value  = rng.gen_range(0..3);
        
        match *top_or_bottom {
            TopOrBottom::Top => pipe.translation.y = top_y_options[chosen_value] as f32,
            TopOrBottom::Bottom => pipe.translation.y = bottom_y_options[chosen_value] as f32,
        }
    }
}
